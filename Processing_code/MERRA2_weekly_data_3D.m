% Weekly average MERRA2 data on specific pressure levels
...
P_use = [850 500 200 100 50 10];
PP = length(P_use);
P_spot = mod(find(lev == P_use),LL);
...
for vv = 1:VV
    ...
    for yy = 1:YY
        ...
        ww = 1;
        wl = 1;
        Data_day = zeros(II,JJ,PP,WL);
        Data_week = zeros(II,JJ,PP,WW);
        ...
        for dd = 1:DD
            ...
            if dd == Week_start(ww+1)
                Data_week(:,:,:,ww) = mean(Data_day,4);
                Data_day = zeros(II,JJ,PP,WL);
                wl = 1;
                ww = ww + 1;
            end

            Data = ncread(file_name,Var{vv});
            Data_day(:,:,:,wl) = mean(Data(:,:,P_spot,:),4);
            wl = wl + 1;
        ...
        for pp = 1:PP
            Var_out = strcat(Var{vv},'_',num2str(P_use(pp)));
            Data_out = squeeze(Data_week(:,:,pp,:));
            nccreate(file_out,Var_out,'Dimensions',{'lon',II,'lat',JJ,'week',WW})
            ncwrite(file_out,Var_out,Data_out)
        end
    end
end
