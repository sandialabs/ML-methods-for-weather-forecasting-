...
def train_and_evaluate_model(train_data, test_data, tuning_df, selected_features, region, horizon, zscore_col, weekly_aves_test):
    ...
    qrfmodel.fit(X_train, y_train)
    
    y_pred = qrfmodel.predict(X_test).T
...
def main():
    ...
    gpfi_df = pd.read_csv("/home/mfholth/subseasonal/weekly_data/Grouped_PFI_datasets/All_GPFI_Merged_EXTREME_QRF.csv")
    ...
    for (region, horizon), group_data in gpfi_df.groupby(["Region", "Horizon"]):
        ...
        print(f"Processing Region: {region}, Horizon: {horizon}")
    ...
    results_df.to_csv("/home/mfholth/subseasonal/weekly_data/rf_feature_importance/iterative_group_rmse_results_QRF_EXTREME.csv", index=False)
