...
def grouped_pfi(model, X_test, y_test, group_dict, metric=mean_squared_error, n_repeats=5, random_state=42):
    ...
    base_pred = model.predict(X_test).T
...
def main():
    os.makedirs("/home/mfholth/subseasonal/weekly_data/Grouped_PFI_QRF", exist_ok=True)
    ...
    for horizon in range(1, 5):  # 1 to 4 weeks ahead
        ...
        for region in regions:
            ...
            group_importance_df.to_csv(f'/home/mfholth/subseasonal/weekly_data/Grouped_PFI_QRF/GPFI_{region}_{horizon}_QRF_cv.csv', index = False)
            group_df.to_csv(f'/home/mfholth/subseasonal/weekly_data/Grouped_PFI_QRF/Groups_{region}_{horizon}_QRF_cv.csv', index = False)
