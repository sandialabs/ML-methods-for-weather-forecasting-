...
fit_regional_inputs_eesns_subset <- 
  function(
    ...
    res_dir <- "../results/preds-BestSubset"
    dir.create(res_dir, showWarnings = FALSE)
    res_subdir <- paste0(res_dir, "/targetregion", toupper(y_region),
                         "-tau", ifelse(tau < 10, paste0(0, tau), tau))
    dir.create(res_subdir, showWarnings = FALSE)
    fp_preds <-
      paste0(res_subdir,
...
compute_rmses_subset <- function(
  ...
  ## Create file path for predictions for this unique set of tuning parameters
  res_dir <- "../results/rmses-BestSubset"
  dir.create(res_dir, showWarnings = FALSE)
  rmse_subdir <- paste0(res_dir, "/targetregion", toupper(y_region),
                       "-tau", ifelse(tau < 10, paste0(0, tau), tau))
  dir.create(rmse_subdir, showWarnings = FALSE)
  preds_subdir <- paste0("../results/preds-BestSubset", "/targetregion", toupper(y_region),
                        "-tau", ifelse(tau < 10, paste0(0, tau), tau))
  ...
  fp_rmses <- paste0(rmse_subdir, run_path)
  fp_preds = paste0(preds_subdir, run_path)
